...
class DARE(nn.Module):
    def __init__(self, backbone, substance):
        ...
        self.classifier_layer = RegressionBranch(input_dim=self._input_dim, substance=substance)
        self.predict_layer = nn.Sequential(self.feature_extractor, self.classifier_layer)
        self.alpha_domain = nn.Parameter(torch.tensor(20.0), requires_grad=True)
